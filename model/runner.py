...
import cv2
import imageio
# from model.model_difflow import output # somehow do not need to use?
import numpy as np
import tensorboardX
import torch
...
class Trainer(object):
    def __init__(
        ...
        if self.workspace is not None:
            if self.use_checkpoint == "scratch":
                ...
                self.load_checkpoint(self.use_checkpoint) ##### call the load_ckpt fn
    ...
    def train_step(self, data):
        ...
        loss_d = self.criterion["depth"](pred_depth, gt_depth)
        loss_r = self.criterion["raydrop"](pred_raydrop, gt_raydrop_smooth)
        loss_i = self.criterion["intensity"](pred_intensity, gt_intensity)        

        lidar_loss = (
            self.opt.alpha_d * loss_d
            + self.opt.alpha_r * loss_r
            + self.opt.alpha_i * loss_i
        )

        # 【新增】记录子项
        loss_details['loss_depth'] = loss_d.detach()
        loss_details['loss_raydrop'] = loss_r.detach()
        loss_details['loss_intensity'] = loss_i.detach()
        ...
        loss_details['loss_chamfer'] = chamfer_loss.detach()
        ...
        if self.opt.flow_loss:
            ...
            # 原本预测的流的shape
            # 与预训练模型预测的流做损失
            # print("pred_flow: ", pred_flow["forward"].shape, pred_flow["backward"].shape) # torch.Size([25964, 3])
            # self.step += 1
            # if self.step == 5:
            #     exit() ##### ????? why exit here? just for testing?
            ...
            for step in [1, 2]: 
                if f"{frame_idx+step}" in self.pc_list.keys():
                    ...
                    # print(f"### pc_forward: {pc_forward.shape}")
                    ...
                    forward_flow = self.SPF_pre_trained(pc, pc_forward, self.device)

                    # print(f"### forward_flow: {forward_flow.shape}, pred_flow_forward: {pred_flow_forward.shape}")
                    forward_flow_loss = l1_loss(forward_flow, pred_flow_forward).mean() ##### 不用mean的话就是一维向量
    
                    loss = loss + forward_flow_loss
                    loss_details['forward_flow_loss'] = forward_flow_loss.detach()
                ...
                if f"{frame_idx-step}" in self.pc_list.keys():
                    ...
                    chamfer_dist = (dist1.sum() + dist2.sum()) * 0.5
                    loss = loss + chamfer_dist
                    backrward_flow = self.SPF_pre_trained(pc, pc_backward, self.device)
                    backward_flow_loss = l1_loss(backrward_flow, pred_flow_backward).mean() #####
                    loss = loss + backward_flow_loss
                    loss_details['backward_flow_loss'] = backward_flow_loss
            ...
            dummyloss = 0.001 * (zero_flow["forward"].abs().sum() + zero_flow["backward"].abs().sum())
            loss = loss + dummyloss
            loss_details['dummyloss'] = dummyloss
        ...
        if self.opt.urf_loss:
            ...
            loss_details['loss_empty'] = loss_empty.detach()
            ...
            loss_details['loss_near'] = loss_near.detach()
        ...
        if patch_size_x > 1:
            ...
            if self.opt.grad_norm_smooth:
                ...
                loss_details['grad_norm_loss'] = grad_norm.detach()
            ...
            if self.opt.spatial_smooth:
                ...
                loss_details['spatial_loss'] = spatial_loss.detach()

            if self.opt.tv_loss:
                tv_loss = torch.mean(dx) + torch.mean(dy)
                # print('tv_loss', tv_loss)
                loss = loss + self.opt.alpha_tv * tv_loss
                loss_details['tv_loss'] = tv_loss.detach()
            ...
            if self.opt.grad_loss:
                ...
                ggrad_loss = self.opt.alpha_grad * grad_loss.sum()
                loss = loss + ggrad_loss
                loss_details['ggrad_loss'] = ggrad_loss
    ...
    # ------------------------------
    # 定义场景流预训练模型（输入点云，输出拼接好的场景流）
    def SPF_pre_trained(self, pc1, pc2, device, batch_size = 8192): 
        '''
        Input: pc N,3
        Ret: the predicted 'ground truth' from difflow model N,3 (N should be the same as pc1.size(0))
        '''
        sf = []
        # print(pc1.shape, pc2.shape) # N可能不一样大, 25964, 25584!! torch.Size([25964, 3]) torch.Size([25584, 3])

        # N = min(pc1.size(0), pc2.size(0)) 
        # @hz_sjtu: if we wanna predict scene flow from pc1 to pc2, we should use N = pc1.size(0)
        N = pc1.size(0)

        gt = torch.zeros(1, batch_size, 3, device = device, dtype = pc1.dtype) # it is ok, since inference only

        # model = self.model.difflow_model()
        model = self.model.difflow_model # without () !!!
        model.eval()

        with torch.no_grad():
            for i in range(0, N, batch_size): # using batch 8192 to divide
                batch_pc1 = pc1[i : i+batch_size, :]
                batch_pc2 = pc2[i : i+batch_size, :]

                if batch_pc1.size(0) < batch_size or batch_pc2.size(0) < batch_size: # padding FIXME why assume residual is same here?
                    pad_size1 = batch_size - batch_pc1.size(0) 
                    pad_size2 = batch_size - batch_pc2.size(0) 
                    assert pad_size1 >= 0 and pad_size2 >= 0, f"pad_size should be non-negative, but got {pad_size1}, {pad_size2}, maybe the pc1's N and pc2's N are not nearly equal!!!"
                    batch_pc1 = torch.cat([batch_pc1, torch.zeros(pad_size1, 3, device = device, dtype = pc1.dtype)])
                    batch_pc2 = torch.cat([batch_pc2, torch.zeros(pad_size2, 3, device = device, dtype = pc2.dtype)])
                
                # check the shape of batch_pc1 and batch_pc2, should be same (required by difflow model)
                assert batch_pc1.size(0) == batch_size and batch_pc2.size(0) == batch_size, f"batch_pc1 and batch_pc2 should have the same size, but got {batch_pc1.size(0)}, {batch_pc2.size(0)}"
                
                # 看看B一般取多大 .view()
                # N,3 -> 1,N,3
                batch_pc1 = batch_pc1.unsqueeze(0)
                batch_pc2 = batch_pc2.unsqueeze(0)

                # model is a PointConvBidirection
                flow, _, _, _, _ = model(batch_pc1, batch_pc2, batch_pc1, batch_pc2, gt) #     flow, _, _, _, _, _ = model(batch_pc1, batch_pc2, batch_pc1, batch_pc2, gt) ValueError: not enough values to unpack (expected 6, got 5)

                # # flow有问题！！
                # a list of flowX: [[flow0_iterN, ..., flow0_iter1], [flow1_iterN, ..., flow1_iter1], [flow2_iterN, ..., flow2_iter1], [flow3]]
                flow = flow[0][0] # 根据论文Fig2主图还有Table12 取最后一层的flow，最精细的

                flow = flow.squeeze(0)
                flow = flow.permute(1, 0)   # N,3
                flow = flow[:min(batch_size, N - i)]    # dim0上去掉padding
                # print("+++++++++++++++++++++++++++", flow.shape) # torch.Size([n, 3])
                # exit()
                sf.append(flow)
            sf = torch.cat(sf, dim = 0) # N, 3          
            return sf
    ...
    def train_one_epoch(self, loader):
        ...
        self.model.train()
        # 保证difflow是eval模式 important
        self.model.difflow_model.eval()
        ...
        for data in loader:
            ...
            #             value = value.sum()
            #             print(f"Component '{key}': value={value.item():.6f}, is_nan={torch.isnan(value).any()}, is_inf={torch.isinf(value).any()}")

            # # 打印最终 loss 的状态
            # print(f"Final Loss: value={loss.item():.6f}, is_nan={torch.isnan(loss).any()}, is_inf={torch.isinf(loss).any()}")
            # print("---------------------------------\n")

            # 如果检测到 NaN 或 Inf，主动报错，方便定位
            if torch.isnan(loss) or torch.isinf(loss):
                # self.log("!!! FATAL: NaN or Inf detected in the loss value. Check the components above. !!!")
                # continue # 可以选择跳过这个batch而不是中断
                raise RuntimeError("!!! FATAL: NaN or Inf detected in the loss value. Check the components above. !!!")
            # =================================================================
            # ======================= 【调试代码结束】 ========================
            # =================================================================
            self.scaler.scale(loss).backward()

            # 2. 在执行 step 之前，unscale 梯度
            self.scaler.unscale_(self.optimizer)

            # 3. 对 unscale 后的梯度进行裁剪
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0) # max_norm=1.0 是一个常用值，你可以根据情况调整
    ...
    #                 filename = f"test_{name}_{i+1:04d}_depth_lidar.npy"
    #                 self.log(f"Frame [{i+1:04d}] ==> {filename}")
    #                 print(f"Frame [{i+1:04d}] ==> {filename}")
    #                 for metric in self.lidar_metrics:
    #                     # RaydropMeter 没有 report_last, 我们跳过它
    #                     if hasattr(metric, 'report_last'):
    #                         last_metric_report = metric.report_last()
    #                         if last_metric_report:
    #                             self.log(f"  - {last_metric_report}")
    #                             print(f"  - {last_metric_report}")
    #             # --- 新增代码块结束 ---
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        if name is None:
            name = f"{self.name}_ep{self.epoch:04d}"
        # Create a dedicated log file for per-sample metrics
        metrics_log_path = os.path.join(self.workspace, "results", f"{name}_metrics.txt")
        os.makedirs(os.path.dirname(metrics_log_path), exist_ok=True)
        metrics_log_ptr = open(metrics_log_path, "w+")
            ...
            for data in loader:
                ...
                for i, metric in enumerate(self.lidar_metrics):
                    if i == 0:  # hard code
                        ...
                        metric.update(preds_depth*preds_mask, gt_depth)
                    #####
                    self.log(f"Sample {self.local_step}: {metric.report_last()}", style="green")
                    if hasattr(metric, 'report_last'):
                        report_str = metric.report_last()
                        metrics_log_ptr.write(f"Sample {self.local_step:04d}: {report_str}\n")
        ...
        # Close the dedicated log file
        metrics_log_ptr.close()
        self.log(f"Per-sample metrics saved to {metrics_log_path}")                
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False):
        ...
        # 确定模型的 state_dict
        # 兼容两种情况：整个文件就是 state_dict，或者 state_dict 在 'model' 键下
        ...
        # --- 核心修改部分 ---
        # 创建一个新的 state_dict，手动去除 torch.compile 可能添加的 '_orig_mod.' 前缀
        # 使用字典推导式可以非常简洁地完成这个操作
        new_state_dict = {
        ...
        # 加载修正后的 state_dict
        missing_keys, unexpected_keys = self.model.load_state_dict( # self.model is kind of LiDAR4D
            new_state_dict, strict=False
        )
        self.log("[INFO] loaded model.")

        missing_keys = [
            key for key in missing_keys if not key.startswith("difflow_model") # ignore the difflow_model part, always load and freeze
        ]
        if len(missing_keys) > 0:
            self.log(f"[WARN] missing keys: {missing_keys}")
        if len(unexpected_keys) > 0:
        ...
        # 如果 checkpoint 中只有模型权重，则提前返回
        if "model" not in checkpoint_dict:
            return

        # --- 以下部分加载优化器、调度器等，保持不变 ---
        if self.ema is not None and "ema" in checkpoint_dict:
            # EMA 的权重也可能需要处理前缀，但通常 EMA 保存时模型是未编译的，所以暂时不加
            self.ema.load_state_dict(checkpoint_dict["ema"])
