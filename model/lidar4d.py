...
class LiDAR4D(LiDAR_Renderer):
    def __init__(
        ...
        **kwargs,
        ...
        # 配置difflow3d的预训练模型
        # model_path = '/home/LiDAR4D/pretrain_model_weights/model_difflow_355_0.0114.pth'
        # model_path = '/home/hz/LiDAR4D/pretrain_model_weights/model_difflow_355_0.0114.pth'
        model_path = '/home/hz/LiDAR4D/pretrain_model_weights/waymo_difflow_591_0.0307.pth'
        # self.difflow_module = importlib.import_module('model_difflow')
        import sys
        sys.path.append('/home/hz/LiDAR4D/model')  # 替换为实际路径
        self.difflow_module = importlib.import_module('model_difflow')
        

        self.difflow_model = getattr(self.difflow_module, 'PointConvBidirection')(iters=4)
        self.difflow_model.load_state_dict(torch.load(model_path))
        print(f"+++ loading pretrained difflow model from: {model_path} +++")
        self.difflow_model.cuda()
        # 冻结参数
        for param in self.difflow_model.parameters():
            param.requires_grad = False

        for m in self.difflow_model.modules():
            if isinstance(m, torch.nn.BatchNorm2d) or isinstance(m, torch.nn.BatchNorm1d):
                m.track_running_stats = False
                m.eval() # 确保BatchNorm处于评估模式
        print("+++ the difflow model was freezed +++")
        self.difflow_model.trainable = False
        self.difflow_model.eval()
    ...
    def density(self, x, t=None):
        ...
        # print(f"!!! DEBUG: Input to trunc_exp (h) stats: min={h.min().item()}, max={h.max().item()}, mean={h.mean().item()}")
        # h = torch.clamp(h, min=-15.0, max=15.0) # 添加此行，将 h 的最大值限制在 15.0 以防止溢出
        # print(f"!!! DEBUG: Input to trunc_exp (h) stats: min={h.min().item()}, max={h.max().item()}, mean={h.mean().item()}")
        sigma = trunc_exp(h[..., 0])
        geo_feat = h[..., 1:]
