...
import sys
sys.path.append('/home/hz/LiDAR4D/model')  # 替换为实际路径
...
class PointConvBidirection(nn.Module):
    ...
    def forward(self, xyz1, xyz2, color1, color2, gt_flow, uncertainty = 0.5):
        # always not training, inference only!
        #xyz1, xyz2: B, N, 3
        #color1, color2: B, N, 3
        ...
        with autocast(enabled=False): ##### 1. encoder
            pc1s, feat1s, idx1s = self.encoder(xyz1.float(), color1.float())
            pc2s, feat2s, idx2s = self.encoder(xyz2.float(), color2.float())
        ...
        with autocast(enabled=False): ##### 2
            pc1s_3_fp32 = pc1s[3].float()
            pc2s_3_fp32 = pc2s[3].float()
            c_feat1_l3_fp32 = c_feat1_l3.float()
            c_feat2_l3_fp32 = c_feat2_l3.float()
            feat1s_3_fp32 = feat1s[3].float()
            feat2s_3_fp32 = feat2s[3].float()
            feat1_new_l3, feat2_new_l3, cross3 = self.cross3(pc1s_3_fp32, pc2s_3_fp32, c_feat1_l3_fp32, c_feat2_l3_fp32, feat1s_3_fp32, feat2s_3_fp32)
        # feat1_new_l3, feat2_new_l3, cross3 = self.cross3(pc1s[3], pc2s[3], c_feat1_l3, c_feat2_l3, feat1s[3], feat2s[3])

        with autocast(enabled=False): ##### 3
            feat3, flow3, certainty3 = self.flow3(pc1s[3].float(), feat1s[3].float(), cross3.float())
        ...
        # return the flow here!
        # flows = np.concatenate((flows0[::-1], flows1[::-1], flows2[::-1], [flow3]))

        # flow3 -> flow0: 256->512->2048->8192
        flows = [flows0[::-1], flows1[::-1], flows2[::-1], [flow3]] # a list of flowX: [[flow0_iterN, ..., flow0_iter1], [flow1_iterN, ..., flow1_iter1], [flow2_iterN, ..., flow2_iter1], [flow3]]
...
# # Mixed Precision Training Compatibility Note:
# # This model uses custom CUDA operators for point cloud processing (PointNet++ operations)
# # including grouping, sampling, ball query, k-nearest neighbors, distance calculation, 
# # and interpolation. These operators may not be compatible with FP16 precision and can
# # cause NaN values or gradient instability. Therefore, we use autocast(enabled=False)
# # to ensure these operations run in FP32 precision while allowing other parts of the
# # model to benefit from mixed precision training.
...
#         # --- Block 1: Encoder ---
#         # PointConvEncoder uses custom CUDA operators (PointConv, PointConvD)
#         # These require FP32 precision for stability
#         with autocast(enabled=False):
#             pc1s, feat1s, idx1s = self.encoder(xyz1.float(), color1.float())
#             pc2s, feat2s, idx2s = self.encoder(xyz2.float(), color2.float())

#         # --- Main Processing Block: All CUDA operators in FP32 ---
#         # This block contains all operations that use custom CUDA operators:
#         # - UpsampleFlow: uses interpolation and sampling operations
#         # - index_points_gather: custom CUDA indexing
#         # - CrossLayer: uses knn_point_cosine for neighbor search
#         # - SceneFlowEstimatorResidual: uses grouping and ball query
#         # - RecurrentUnit: contains BidirectionalLayer, FlowEmbedding, PointWarping, 
#         #   and DiffusionSceneFlowGRUResidual (which uses square_distance and index_points_group)
#         with autocast(enabled=False):
#             #l4 -> l3 upsample
#             feat1_l4_3 = self.upsample(pc1s[3].float(), pc1s[4].float(), feat1s[4].float())
#             feat1_l4_3 = self.deconv4_3(feat1_l4_3)
#             feat2_l4_3 = self.upsample(pc2s[3].float(), pc2s[4].float(), feat2s[4].float())
#             feat2_l4_3 = self.deconv4_3(feat2_l4_3)
            
#             # GT flow indexing (uses custom CUDA indexing)
#             l2_label = index_points_gather(gt_flow.float(), idx1s[1])
#             l1_label = index_points_gather(gt_flow.float(), idx1s[0])
#             l0_label = gt_flow.float()

#             #l3 processing
#             c_feat1_l3 = torch.cat([feat1s[3], feat1_l4_3], dim = 1)
#             c_feat2_l3 = torch.cat([feat2s[3], feat2_l4_3], dim = 1)
            
#             # Cross layer and flow estimation (use custom CUDA operators)
#             feat1_new_l3, feat2_new_l3, cross3 = self.cross3(
#                 pc1s[3].float(), pc2s[3].float(), 
#                 c_feat1_l3.float(), c_feat2_l3.float(), 
#                 feat1s[3].float(), feat2s[3].float()
#             )
#             feat3, flow3, certainty3 = self.flow3(pc1s[3].float(), feat1s[3].float(), cross3.float())

#             # l3 -> l2 upsample
#             feat1_l3_2 = self.upsample(pc1s[2].float(), pc1s[3].float(), feat1_new_l3.float())
#             feat1_l3_2 = self.deconv3_2(feat1_l3_2)
#             feat2_l3_2 = self.upsample(pc2s[2].float(), pc2s[3].float(), feat2_new_l3.float())
#             feat2_l3_2 = self.deconv3_2(feat2_l3_2)

#             #l2 processing
#             up_flow2 = self.upsample(pc1s[2].float(), pc1s[3].float(), (self.scale * flow3).float())
#             up_certainty2 = self.upsample(pc1s[2].float(), pc1s[3].float(), (self.scale * certainty3).float())
#             up_feat2 = self.upsample(pc1s[2].float(), pc1s[3].float(), feat3.float())
            
#             # Recurrent unit l2 (contains multiple CUDA operators)
#             if self.training:
#                 flows2, feat1_new_l2, feat2_new_l2, feat2, certainty2, loss_l2 = self.recurrent2(
#                     pc1s[2].float(), pc2s[2].float(), 
#                     feat1_l3_2.float(), feat2_l3_2.float(), 
#                     feat1s[2].float(), feat2s[2].float(), 
#                     up_flow2.float(), up_feat2.float(), 
#                     l2_label.float(), up_certainty2.float(), uncertainty
#                 )
#             else:
#                 flows2, feat1_new_l2, feat2_new_l2, feat2, certainty2 = self.recurrent2(
#                     pc1s[2].float(), pc2s[2].float(), 
#                     feat1_l3_2.float(), feat2_l3_2.float(), 
#                     feat1s[2].float(), feat2s[2].float(), 
#                     up_flow2.float(), up_feat2.float(), 
#                     None, up_certainty2.float(), uncertainty
#                 )

#             # l2 -> l1 upsample
#             feat1_l2_1 = self.upsample(pc1s[1].float(), pc1s[2].float(), feat1_new_l2.float())
#             feat1_l2_1 = self.deconv2_1(feat1_l2_1)
#             feat2_l2_1 = self.upsample(pc2s[1].float(), pc2s[2].float(), feat2_new_l2.float())
#             feat2_l2_1 = self.deconv2_1(feat2_l2_1)

#             #l1 processing
#             up_flow1 = self.upsample(pc1s[1].float(), pc1s[2].float(), (self.scale * flows2[-1]).float())
#             up_certainty1 = self.upsample(pc1s[1].float(), pc1s[2].float(), (self.scale * certainty2).float())
#             up_feat1 = self.upsample(pc1s[1].float(), pc1s[2].float(), feat2.float())
            
#             # Recurrent unit l1 (contains multiple CUDA operators)
#             if self.training:
#                 flows1, feat1_new_l1, feat2_new_l1, feat1, certainty1, loss_l1 = self.recurrent1(
#                     pc1s[1].float(), pc2s[1].float(), 
#                     feat1_l2_1.float(), feat2_l2_1.float(), 
#                     feat1s[1].float(), feat2s[1].float(), 
#                     up_flow1.float(), up_feat1.float(), 
#                     l1_label.float(), up_certainty1.float(), uncertainty
#                 )
#             else:
#                 flows1, feat1_new_l1, feat2_new_l1, feat1, certainty1 = self.recurrent1(
#                     pc1s[1].float(), pc2s[1].float(), 
#                     feat1_l2_1.float(), feat2_l2_1.float(), 
#                     feat1s[1].float(), feat2s[1].float(), 
#                     up_flow1.float(), up_feat1.float(), 
#                     None, up_certainty1.float(), uncertainty
#                 )

#             # l1 -> l0 upsample
#             feat1_l1_0 = self.upsample(pc1s[0].float(), pc1s[1].float(), feat1_new_l1.float())
#             feat1_l1_0 = self.deconv1_0(feat1_l1_0)
#             feat2_l1_0 = self.upsample(pc2s[0].float(), pc2s[1].float(), feat2_new_l1.float())
#             feat2_l1_0 = self.deconv1_0(feat2_l1_0)

#             #l0 processing
#             up_flow0 = self.upsample(pc1s[0].float(), pc1s[1].float(), (self.scale * flows1[-1]).float())
#             up_certainty0 = self.upsample(pc1s[0].float(), pc1s[1].float(), (self.scale * certainty1).float())
#             up_feat0 = self.upsample(pc1s[0].float(), pc1s[1].float(), feat1.float())
            
#             # Recurrent unit l0 (contains multiple CUDA operators)
#             if self.training:
#                 flows0, feat1_new_l0, feat2_new_l0, feat0, certainty0, loss_l0 = self.recurrent0(
#                     pc1s[0].float(), pc2s[0].float(), 
#                     feat1_l1_0.float(), feat2_l1_0.float(), 
#                     feat1s[0].float(), feat2s[0].float(), 
#                     up_flow0.float(), up_feat0.float(), 
#                     l0_label.float(), up_certainty0.float(), uncertainty
#                 )
#             else:
#                 flows0, feat1_new_l0, feat2_new_l0, feat0, certainty0 = self.recurrent0(
#                     pc1s[0].float(), pc2s[0].float(), 
#                     feat1_l1_0.float(), feat2_l1_0.float(), 
#                     feat1s[0].float(), feat2s[0].float(), 
#                     up_flow0.float(), up_feat0.float(), 
#                     None, up_certainty0.float(), uncertainty
#                 )
