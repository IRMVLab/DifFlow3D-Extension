...
class DepthMeter:
    ...
    def report_last(self):
        if not self.V:
            return "Depth_error = No data"
        return f"Depth_error = {np.array(self.V[-1])}"
    # def report_last(self):
    #     if not self.preds:
    #         return ""
    #     pred = self.preds[-1]
    #     truth = self.truths[-1]
    #     rmse, medae, lpips_loss, ssim, psnr = self.compute_depth_errors(truth, pred)
    #     return f"Depth: RMSE={rmse:.4f}, MedAE={medae:.4f}, LPIPS={lpips_loss:.4f}, SSIM={ssim:.4f}, PSNR={psnr:.4f}"
...
class IntensityMeter:
    ...
    #     pred = self.preds[-1]
    #     truth = self.truths[-1]
    #     rmse, medae, lpips_loss, ssim, psnr = self.compute_intensity_errors(truth, pred)
    #     return f"Intensity: RMSE={rmse:.4f}, MedAE={medae:.4f}, LPIPS={lpips_loss:.4f}, SSIM={ssim:.4f}, PSNR={psnr:.4f}"
    def report_last(self):
        if not self.V:
            return "Intensity_error = No data"
        return f"Intensity_error = {np.array(self.V[-1])}"
...
class RaydropMeter:
    ...
    def report_last(self):
        if not self.V:
            return "Raydrop_error = No data"
        return f"Raydrop_error = {np.array(self.V[-1])}"
    # def report_last(self):
    #     if not self.preds:
    #         return ""
        
    #     chamLoss = chamfer_3DDist()
    #     pred_lidar = pano_to_lidar(self.preds[-1][0], self.intrinsics)
    #     gt_lidar = pano_to_lidar(self.truths[-1][0], self.intrinsics)
    ...
    #     threshold = 0.05
    #     f_score, precision, recall = fscore(dist1, dist2, threshold)
    #     return f"Points: CD={chamfer_dis:.4f}, F-score={f_score:.4f}, Precision={precision:.4f}, Recall={recall:.4f}"
...
class PointsMeter:
    ...
    def report_last(self):
        if not self.V:
            return "Points_error = No data"
        return f"Points_error = {np.array(self.V[-1])}"
