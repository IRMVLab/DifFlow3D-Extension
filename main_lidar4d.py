...
torch.set_float32_matmul_precision('high') # for 4090 / a100 better perf
...
def get_arg_parser():
    ...
    parser.add_argument("--fp16", type=bool, default=False, help="use amp mixed precision training") ##### note that we do not want to set it to True, since pointnet2 does not support fp16??
...
def main():
    ...
    if opt.test or opt.test_eval or opt.refine:
        ...
        # if opt.ckpt is a file
        # then max epoch += 100
        if opt.ckpt and os.path.isfile(opt.ckpt):
            max_epoch += 200
            print(f"Checkpoint file provided, extending training to max_epoch: {max_epoch}! (200 more from original lidar4d model training)")
        if trainer.epoch > max_epoch: 
            max_epoch = trainer.epoch + 200
            print(f"Checkpoint epoch {trainer.epoch} already exceeds max_epoch, extending training to max_epoch: {max_epoch}! (200 more from current epoch)")
        # trainer.model.difflow_model = torch.compile(trainer.model.difflow_model)
        trainer.model = torch.compile(trainer.model)
        print("compile difflow done!")
